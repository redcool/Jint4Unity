            ...
            if (valueType == typeof(Func<JsValue, JsValue[], JsValue>))
            {
                ...
                if (type.IsGenericType)
                {
                    ...
                    if (genericType.Name.StartsWith("Action"))
                    {
                        ...
                        Expression callExpresion = Expression.Call(
                    ...
                    else if (genericType.Name.StartsWith("Func"))
                    {
                        ...
                        {
                            var boxingExpr = Expression.Convert(item, typeof(object));
                            var methodCallExpr = Expression.Call(null, jsValueFromObject, Expression.Constant(_engine, typeof(Engine)), boxingExpr);
                            list.Add(methodCallExpr);
                        }
                        var @vars = Expression.NewArrayInit(typeof(JsValue),list.ToArray());
                ...
                else
                {
                    ...
                    else if (type.IsSubclassOf(typeof(System.MulticastDelegate)))
                    {
                        ...
                        var list = new List<Expression>();
                        var ite = @params.Select(p => Expression.Call(null, typeof(JsValue).GetMethod("FromObject"), Expression.Constant(_engine, typeof(Engine)), p));
                        foreach (var item in ite)
                        {
                            list.Add(item);
                        }

                        var @vars = Expression.NewArrayInit(typeof(JsValue), list);
